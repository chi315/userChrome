location == "chrome://browser/content/browser.xul" && (function(){
	...
	btn.id = "extensionButton";
	btn.setAttribute("label", "extension");
	btn.setAttribute("type", "menu");
	btn.setAttribute("tooltiptext", "extension");
	btn.appendChild(document.createElement("menupopup")).addEventListener("popupshowing", showExtensionList,false)
	...
	Array.slice(toolbars).forEach(function (toolbar) {
		...
		if (currentset.split(",").indexOf("extensionButton") < 0) return;
	...
	function showExtensionList(node){
		...
		AddonManager.getAddonsByTypes(["extension"], function(extension){
			...
			for(var i=0;i<extension.length;i++){
				...
				item[i].setAttribute("label",extension[i].name);
