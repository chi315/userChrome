...
// 放置的位置，true為地址欄，false為附加組件欄（可移動按鈕）
var isUrlbar = true;
var ORIGINAL_SITEINFO = true;  // 原版規則是否啟用？略大，但很多站點都可以翻頁
var SIMPLE_SEPARATOR = false;  // true 原版的分隔條，false 多功能分隔條

var DB_FILENAME_CN =  "uSuper_preloader.db.js";   // 中文數據庫的位置
var DB_FILENAME_JSON = "uAutoPagerize.json";      // 原版數據庫位置
...
var IMMEDIATELY_PAGER_NUM = 100;    // 立即加載的初始頁數
var USE_IFRAME = true;            // 是否啟用 iframe 加載下一頁（瀏覽器級，默認只允許JavaScript，在 createIframe 中可設置其它允許）
var USE_FORCE_NEXT_PAGE = true;   // 是否啟用強制拼接？
var PRE_REQUEST_NEXT_PAGE = true; // 提前預讀下一頁..就是翻完第1頁,立馬預讀第2頁,翻完第2頁,立馬預讀第3頁..(大幅加快翻頁快感-_-!!)
...
// 分頁導航上面的橫線，背景在下面的 background 修改
...
var ns = window.uAutoPagerize = {
    ...
    addButton: function(){
        var icon;
        if(isUrlbar){
            icon = $('TabsToolbar').appendChild($C("image", {
        ...
        var xml = '\
            <menupopup id="uAutoPagerize-popup"\
                ...
                <menuitem label="更新中文規則" \
                          tooltiptext="包含Super_preloader規則和本人維護的規則"\
                ...
                <menuitem label="強制拼接" type="checkbox" disabled="true" \
    ...
    onPopupShowing: function(event){
        ...
        if(excludeStr){
            ...
            blacklistText.setAttribute("tooltiptext", "修改黑名單或從中刪除，空白為刪除");
            ...
            blacklistText.setAttribute("tooltiptext", "添加到黑名單中");
    ...
    loadSetting: function(isAlert) {
        ...
        try {
            ...
            log('load error.', e);
            return alerts('uAutoPagerize', '配置文件有錯誤');
        }
        ...
        if (isAlert)
            alerts('uAutoPagerize', '配置文件已經重新載入');
    ...
    launch: function(win, timer){
        ...
        // 還得加上nextLink，不加上的話會查找下一頁鏈接 2 次，特別是加了自動查找功能後
        ...
        win.setTimeout(function(){
            ...
            debug("---------------------------------------------------------");
            debug("開始查找規則：" + locationHref);

            if (!info) [, info, nextLink] = ns.getInfo(ns.MY_SITEINFO, win);

            // 第二檢測 Super_preloader.db 的數據庫
            ...
            let spenTime = "耗時: "+ (new Date().getTime() - startTime) + 'ms. ';
            if (info) {
                debug("  找到匹配當前站點的規則: 是[" + info.type + ']第' + (index + 1) + '規則，' + spenTime +
                    "開始運行 Autopager");
                ...
                debug("  所有規則都沒法找到下一頁或內容，" + "共查找規則" + infoLength + "個，" + spenTime +
                    "結束運行");
            }
        ...
        for (let [index, info] in Iterator(list)) {
            try {
                ...
                if (info.url.length > 12)
                    debug("  匹配地址的規則：[" + (info.type || "") + "]" + "第" + (index + 1) + "個");
...
AutoPager.prototype = {
    ...
    doRequest: function(){
        ...
        if(this.iframeMode){
            this.win.setTimeout(function(self){  // 延時點
    ...
    getRalativePageStr : function () {
        ...
        if (realPageSiteMatch) { //如果匹配就顯示實際網頁信息
            ...
            {
                var ralativePageStr = ' [ 實際：第 <font color="red">' + ralativePageNumarray[0] + ' - ' + ralativePageNumarray[1] + '</font> 項 ]';
            } else if ((ralativePageNumarray[1] - ralativePageNumarray[0]) == 1) //一般的翻頁數，差值應該是1
            {
                var ralativePageStr = ' [ 實際：第 <font color="red">' + ralativePageNumarray[0] + '</font> 頁 ]';
            } else if ((ralativePageNumarray[0] == 0 && ralativePageNumarray[1]) == 0) //找不到的話……
            {
                var ralativePageStr = ' [ 實際網頁結束 ]';
            }
        } else {
            ralativePageStr = '';
        }
...
(function(){
    ...
    ns.loadSetting_CN = function(isAlert) {
        ...
        debug("Super_preloader 及其它規則已經載入")

        if (isAlert)
            alerts('uAutoPagerize', '中文規則已經重新載入');
...
// 修改過，為了能使用 responseType = "document"，這樣下一頁頭像和 xpath 獲取 body 都有能用。
