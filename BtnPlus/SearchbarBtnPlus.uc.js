...
//	}, true);
	...
	searchPopup.setAttribute("onclick", "if (event.button !== 0) {event.preventDefault(); event.stopPropagation();}");
	...
	searchbarEngineBtn.setAttribute("onclick", "if (event.button == 0) {SearchEngine.popupClick(event);} else if (event.button == 1) {gBrowser.addTab(readFromClipboard());} else if (event.button == 2) {gBrowser.selectedTab = gBrowser.addTab(readFromClipboard()); event.preventDefault();}");
